...
Fork the [uniconv/registry](https://github.com/uniconv/registry) repo and create a PR adding `plugins/my-plugin/plugin.json`:
...
There are two different `plugin.json` files:

| File | Location | Purpose |
|------|----------|---------|
| **Local manifest** | Inside the plugin directory | Tells uniconv how to load and run the plugin |
| **Registry manifest** | In the registry repo under `plugins/<name>/` | Tells uniconv how to download and install the plugin |

The local manifest is what ships inside the tarball. The registry manifest is what you submit via PR.
